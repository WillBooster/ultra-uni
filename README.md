...
`format` removes trailing whitespace outside multi-line literals and trailing blank lines, converts CRLF line endings outside literals to LF, and ends non-empty code with a newline, but never changes the content of a literal or of Ruby's `__END__` data, even one that reaches the end of the file. It throws when the code has syntax errors.
